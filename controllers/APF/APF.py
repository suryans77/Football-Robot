from controller import Supervisor
import numpy as np
import math

# ================= INITIALIZATION =================
robot = Supervisor()
timestep = int(robot.getBasicTimeStep())

# Supervisor handles for defenders
def1 = robot.getFromDef("defender1")
def2 = robot.getFromDef("defender2")
def3 = robot.getFromDef("defender3") # Optional third defender, can be None
ball_node = robot.getFromDef("Ball")

# Standard devices
gps = robot.getDevice("gps")
gps.enable(timestep)
compass = robot.getDevice("compass")
compass.enable(timestep)

# Motors
left_motor = robot.getDevice("left wheel motor")
right_motor = robot.getDevice("right wheel motor")
left_motor.setPosition(float('inf'))
right_motor.setPosition(float('inf'))

# ================= DEBUG SWITCH =================
DEBUG = False

# ================= PARAMETERS =================
GOAL_POS = [2.0, 0.0]

# VRF (Virtual Range Finder) Parameters
NUM_BEAMS = 20
MAX_RANGE = 2.0
VRF_SENSORS = [MAX_RANGE] * NUM_BEAMS

# APF Gains
K_ATTRACT = 2.0
K_REPEL = 0.8
REPULSION_RADIUS = 0.5

# Speed Parameters
MAX_SPEED = 21.0
DRIBBLE_SPEED = 10.0  
APPROACH_SPEED = 15.0

# Dribbler Parameters (Transplanted from Teleop)
DRIBBLER_OFFSET = 0.05  
DRIBBLER_PULL = 15.0    

# Turning Parameters
MAX_TURN_RATE = 4.0    
ANGLE_TOLERANCE = 0.2  

# Escape Mechanism
ESCAPE_GAIN = 0.6
MIN_FORCE_THRESHOLD = 0.08
escape_timer = 0

# Heading smoothing
smoothed_heading = 0.0

# ================= FUNCTIONS =================
def get_robot_heading(compass_values):
    return math.atan2(compass_values[0], compass_values[1])

def update_vrf(curr_pos):
    global VRF_SENSORS
    VRF_SENSORS = [MAX_RANGE] * NUM_BEAMS
    
    dynamic_obstacles = []
    if def1 is not None: dynamic_obstacles.append(def1.getPosition()[:2])
    if def2 is not None: dynamic_obstacles.append(def2.getPosition()[:2])
    if def3 is not None: dynamic_obstacles.append(def3.getPosition()[:2])
    
    for obs in dynamic_obstacles:
        dx = obs[0] - curr_pos[0]
        dy = obs[1] - curr_pos[1]
        dist = math.hypot(dx, dy)
        
        if dist < MAX_RANGE:
            angle = math.atan2(dy, dx)
            idx = int((angle + math.pi) / (2 * math.pi) * NUM_BEAMS) % NUM_BEAMS
            VRF_SENSORS[idx] = min(VRF_SENSORS[idx], dist)

def calculate_apf_force(curr_pos):
    global escape_timer
    
    # 1. Goal Attraction
    dx_g = GOAL_POS[0] - curr_pos[0]
    dy_g = GOAL_POS[1] - curr_pos[1]
    dist_g = math.hypot(dx_g, dy_g)
    
    fx = (dx_g / max(0.01, dist_g)) * K_ATTRACT
    fy = (dy_g / max(0.01, dist_g)) * K_ATTRACT
    
    # 2. VRF-based Repulsion
    for i in range(NUM_BEAMS):
        d = VRF_SENSORS[i]
        if d < REPULSION_RADIUS:
            angle = (i / NUM_BEAMS) * 2 * math.pi - math.pi
            repel_mag = K_REPEL * (1.0/max(0.1, d) - 1.0/REPULSION_RADIUS)
            
            fx -= math.cos(angle) * repel_mag
            fy -= math.sin(angle) * repel_mag
            
    # 3. Escape Mechanism
    force_mag = math.hypot(fx, fy)
    if force_mag < MIN_FORCE_THRESHOLD:
        escape_timer += 1
    else:
        escape_timer = 0
        
    if escape_timer > 10:
        random_angle = np.random.uniform(-math.pi, math.pi)
        fx += ESCAPE_GAIN * math.cos(random_angle)
        fy += ESCAPE_GAIN * math.sin(random_angle)
        escape_timer = 0
        
    return fx, fy

# ================= MAIN LOOP =================
state = "APPROACH"

while robot.step(timestep) != -1:
    if ball_node is None: continue
    
    # ---- Sensors ----
    gps_vals = gps.getValues()
    if math.isnan(gps_vals[0]): continue
    
    curr_pos = [gps_vals[0], gps_vals[1]]
    ball_pos = ball_node.getPosition()
    
    # Smooth heading
    raw_heading = get_robot_heading(compass.getValues())
    smoothed_heading = (0.9 * smoothed_heading) + (0.1 * raw_heading)
    heading = smoothed_heading
    
    # ---- Dribbler Check ----
    dx = ball_pos[0] - curr_pos[0]
    dy = ball_pos[1] - curr_pos[1]
    dist_to_ball = math.hypot(dx, dy)
    
    angle_to_ball = math.atan2(dy, dx)
    angle_diff = angle_to_ball - heading
    while angle_diff > math.pi: angle_diff -= 2 * math.pi
    while angle_diff < -math.pi: angle_diff += 2 * math.pi
    
    # Check if ball is close enough and in front to snap into the dribbler
    is_dribbling = (dist_to_ball < 0.1 and abs(angle_diff) < 0.5)
    
    # ---- State Machine ----
    if is_dribbling:
        state = "NAVIGATE"
    else:
        state = "APPROACH"
        
    update_vrf(curr_pos)
    
    # ---- Control Logic ----
    if state == "NAVIGATE":
        # 1. Steer via APF
        fx, fy = calculate_apf_force(curr_pos)
        target_angle = math.atan2(fy, fx)
        
        angle_error = target_angle - heading
        while angle_error > math.pi: angle_error -= 2*math.pi
        while angle_error < -math.pi: angle_error += 2*math.pi
        
        turn_gain = 6.0
        turn = max(min(angle_error * turn_gain, MAX_TURN_RATE), -MAX_TURN_RATE)
        
        turn_ratio = abs(turn) / MAX_TURN_RATE
        base_speed = DRIBBLE_SPEED * (1.0 - 0.65 * turn_ratio)
        
        if abs(angle_error) > ANGLE_TOLERANCE:
            base_speed *= 0.7  
            
        left_speed = base_speed - turn
        right_speed = base_speed + turn

        # 2. APPLY PSEUDO-DRIBBLER PHYSICS
        notch_x = curr_pos[0] + math.cos(heading) * DRIBBLER_OFFSET
        notch_y = curr_pos[1] + math.sin(heading) * DRIBBLER_OFFSET
        
        pull_vx = (notch_x - ball_pos[0]) * DRIBBLER_PULL
        pull_vy = (notch_y - ball_pos[1]) * DRIBBLER_PULL
        ball_node.setVelocity([pull_vx, pull_vy, 0.0, 0.0, 0.0, 0.0])

        if DEBUG:
            print(f"NAVIGATE | APF Ang: {math.degrees(target_angle):.1f}° | Turn: {turn:.2f}")

    elif state == "APPROACH":
        # Steer purely toward the exact GPS coordinate of the ball
        turn_gain = 8.0
        turn = max(min(angle_diff * turn_gain, MAX_TURN_RATE), -MAX_TURN_RATE)
        
        # Slow down slightly if we need to turn hard to approach it
        base_speed = APPROACH_SPEED * max(0.2, 1.0 - (abs(angle_diff) / math.pi))
        
        left_speed = base_speed - turn
        right_speed = base_speed + turn
        
        if DEBUG:
            print(f"APPROACH | Dist: {dist_to_ball:.2f}m | Turn: {turn:.2f}")

    # ---- Actuate Motors ----
    left_speed = max(min(left_speed, MAX_SPEED), -MAX_SPEED)
    right_speed = max(min(right_speed, MAX_SPEED), -MAX_SPEED)
    
    left_motor.setVelocity(left_speed)
    right_motor.setVelocity(right_speed)