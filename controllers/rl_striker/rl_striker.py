...
class StrikerRLEnv(gym.Env):
    def __init__(self):
        ...
        # Action Space: [Gas Pedal, Steering Wheel, Shoot Flag]
        ...
        self.v_curr = 0.0
        self.step_count = 0
        self.prev_ball_dist = 0.0 # Used for Delta Reward
    ...
    def reset(self, seed=None, options=None):
        ...
        # Curriculum: Spawn ball 15cm directly in front
        ...
        # Set the initial distance to the goal for our Delta Reward
    ...
    def step(self, action):
        ...
        # --- FIX 1: GAS PEDAL & STEERING MATH ---
        forward_speed = action[0] * self.MAX_SPEED        # Drives forward/backward
        turn_speed = action[1] * (self.MAX_SPEED * 0.5)   # Steers left/right
        
        left_target = forward_speed - turn_speed
        right_target = forward_speed + turn_speed
        
        # Clamp to max motor speeds just in case
        left_target = max(min(left_target, 20.0), -20.0)
        right_target = max(min(right_target, 20.0), -20.0)
        
        self.left_motor.setVelocity(left_target)
        self.right_motor.setVelocity(right_target)
        self.v_curr = (left_target + right_target) / 2.0
        
        # ACTION REPEAT WITH PSEUDO-DRIBBLER
        for _ in range(8):
            pos = self.gps.getValues()
            comp = self.compass.getValues()
            heading = np.arctan2(comp[0], comp[1])
        ...
        pos = self.gps.getValues()
        ball_pos = self.ball_node.getPosition()
        dist_robot_to_ball = np.hypot(ball_pos[0] - pos[0], ball_pos[1] - pos[1])
        dist_ball_to_goal = np.hypot(self.MY_GOAL_CENTER[0] - ball_pos[0], self.MY_GOAL_CENTER[1] - ball_pos[1])
        
        reward = 0.0
        done = False
        
        # --- FIX 2: THE DELTA REWARD ---
        # Did the ball get closer to the goal since the last frame?
        progress = self.prev_ball_dist - dist_ball_to_goal
        
        # If progress is positive (moving forward), massive reward!
        # If progress is negative (moving backward), penalty!
