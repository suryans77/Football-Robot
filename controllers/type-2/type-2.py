from controller import Supervisor
import math

# 1. Initialize
robot = Supervisor()
timestep = int(robot.getBasicTimeStep())

gps = robot.getDevice("gps")
gps.enable(timestep)
compass = robot.getDevice("compass")
compass.enable(timestep)

left_motor = robot.getDevice("left wheel motor")
right_motor = robot.getDevice("right wheel motor")
left_motor.setPosition(float('inf'))
right_motor.setPosition(float('inf'))

striker_node = robot.getFromDef("Striker")

# --- GAME PARAMETERS ---
MY_GOAL_CENTER = [2.0, 0.0]
DEFENSE_RADIUS = 0.6  # Distance from goal center to position the defender

# --- MOVEMENT PARAMETERS ---
WHEEL_RADIUS = 0.02
AXLE_LENGTH = 0.052
LINEAR_SPEED = 0.36   # m/s
ANGULAR_SPEED = 5.0   # rad/s

# PD Steering Constants  (same as type-1)
Kp = 4.0
Kd = 1.2
prev_error = 0.0

ACCEL = 0.2
base_speed_curr = 0.0
turn_curr = 0.0

while robot.step(timestep) != -1:
    if striker_node is None:
        continue

    # ── Sensor data ───────────────────────────────────────────────────
    curr_pos    = gps.getValues()
    striker_pos = striker_node.getPosition()
    striker_compass = striker_node.getOrientation()
    compass_val = compass.getValues()
    my_heading  = math.atan2(compass_val[0], compass_val[1])

    fx = striker_compass[0]   # r00
    fy = striker_compass[3]   # r10

    striker_pos[0] += 0.06 * fx
    striker_pos[1] += 0.06 * fy

    # ── STRATEGY: compute ideal blocking spot (unchanged from original) ─
    vec_x = striker_pos[0] - MY_GOAL_CENTER[0]
    vec_y = striker_pos[1] - MY_GOAL_CENTER[1]
    dist_goal_to_striker = math.hypot(vec_x, vec_y)

    if dist_goal_to_striker < 0.001:
        unit_x, unit_y = 1.0, 0.0
    else:
        unit_x = vec_x / dist_goal_to_striker
        unit_y = vec_y / dist_goal_to_striker

    target_x = MY_GOAL_CENTER[0] + (unit_x * DEFENSE_RADIUS)
    target_y = MY_GOAL_CENTER[1] + (unit_y * DEFENSE_RADIUS)

    # ── Navigation vectors ────────────────────────────────────────────
    dx = target_x - curr_pos[0]
    dy = target_y - curr_pos[1]
    dist_to_target = math.hypot(dx, dy)

    angle_to_spot    = math.atan2(dy, dx)
    angle_to_striker = math.atan2(
        striker_pos[1] - curr_pos[1],
        striker_pos[0] - curr_pos[0],
    )

    if dist_to_target < 0.05:
        # Stay in position, just face striker
        base_speed = 0.0
        target_angle = angle_to_striker

    else:
        # Drive toward blocking spot
        target_angle = angle_to_spot

        if dist_to_target > 0.15:
            base_speed = LINEAR_SPEED
        else:
            base_speed = LINEAR_SPEED * (dist_to_target / 0.15)
            base_speed = max(base_speed, 0.1)

    # ── PD Steering ───────────────────────────────────────────────────
    angle_error = target_angle - my_heading
    while angle_error >  math.pi: angle_error -= 2 * math.pi
    while angle_error < -math.pi: angle_error += 2 * math.pi

    error_rate   = angle_error - prev_error
    turn_amount  = (Kp * angle_error) + (Kd * error_rate)
    turn_amount  = max(min(turn_amount, ANGULAR_SPEED), -ANGULAR_SPEED)
    prev_error   = angle_error

    if abs(angle_error) < 0.5:
        speed_multiplier = 1.0
    else:
        speed_multiplier = max(0.0, 1.0 - (abs(angle_error) / (math.pi / 2)))

    current_base_speed = base_speed * speed_multiplier

    # ── Smoothing & inverse kinematics ───────────────────────────────
    base_speed_curr += (current_base_speed - base_speed_curr) * ACCEL
    turn_curr       += (turn_amount        - turn_curr)       * ACCEL

    left_speed  = (base_speed_curr - (turn_curr * AXLE_LENGTH / 2.0)) / WHEEL_RADIUS
    right_speed = (base_speed_curr + (turn_curr * AXLE_LENGTH / 2.0)) / WHEEL_RADIUS

    left_speed  = max(min(left_speed,  21), -21)
    right_speed = max(min(right_speed, 21), -21)

    left_motor.setVelocity(left_speed)
    right_motor.setVelocity(right_speed)